...
-- tugas
-- nomer 1 
CREATE VIEW Anggota_Lebih_Lima_Peminjaman AS
...
SELECT * FROM Anggota_Lebih_Lima_Peminjaman;
-- nomer 2
CREATE VIEW Transaksi_Petugas AS 
SELECT p.IdPetugas, pt.Nama, 
       COUNT(p.kode_peminjaman) AS Jumlah_Peminjaman,
       COUNT(pg.Kode_Kembali) AS Jumlah_Pengembalian,
       COUNT(p.Kode_Buku) AS Jumlah_Buku_Terpinjam
FROM peminjaman p JOIN petugas pt ON p.IdPetugas = pt.IdPetugas LEFT JOIN pengembalian pg ON p.kode_peminjaman = pg.id_peminjaman
GROUP BY p.IdPetugas, pt.Nama;
...
FROM peminjaman p
JOIN petugas pt ON p.IdPetugas = pt.IdPetugas
LEFT JOIN pengembalian pg ON p.kode_peminjaman = pg.id_peminjaman
GROUP BY p.IdPetugas, pt.Nama
HAVING COUNT(DISTINCT p.kode_peminjaman) = (
    SELECT MAX(transaksi) 
    FROM (
        SELECT COUNT(DISTINCT kode_peminjaman) AS transaksi 
        FROM peminjaman
        GROUP BY IdPetugas
    ) AS TransaksiPetugas
);
...
SELECT p.Kode_Buku, b.judul_buku AS Judul_Buku, COUNT(*) AS Jumlah_Peminjaman
FROM peminjaman p
