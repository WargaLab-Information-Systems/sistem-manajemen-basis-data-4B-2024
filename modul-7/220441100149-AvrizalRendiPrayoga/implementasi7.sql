...
-- 1
DELIMITER // 
CREATE TRIGGER validasiRencanaKembali
...
INSERT INTO peminjaman VALUES (6, 2, 4, CURDATE(), '2024-05-25', 5, 4000000.00, '2024-05-25', 0.00);

-- 2
DELIMITER //
CREATE TRIGGER perhitunganTotalBayar
    ...
    IF NEW.tgl_kembali IS NOT NULL THEN
        ...
        SET NEW.denda = CASE
			...
			ELSE 
				0
			END;                  
        SET NEW.total_bayar = (DATEDIFF( NEW.tgl_kembali, NEW.tgl_pinjam)) * (SELECT harga_sewa_perhari 
										FROM mobil 
										WHERE id_mobil = NEW.id_mobil) + NEW.denda;
    END IF;
END //
DELIMITER ;
DROP TRIGGER perhitunganTotalBayar;
UPDATE peminjaman SET tgl_kembali = '2024-05-29' WHERE id_peminjaman = 6;

-- 3
DELIMITER //
CREATE TRIGGER validasiPanjangNIK
...
INSERT INTO Pelanggan VALUES (6, 'Anonim', 'Jl. gagak No. 95', '123456789123456789', '085144441155', 'L');

-- 4
DELIMITER //
CREATE TRIGGER validasiPlatNo
...
END //
DELIMITER ;
DROP TRIGGER validasiPlatNo
INSERT INTO Mobil VALUES (6, 'A234XYZ', 'Toyota', 'Sedan', 500000);